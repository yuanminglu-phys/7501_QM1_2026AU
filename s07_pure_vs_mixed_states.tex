...
Similar to the case of pure states, in a composite system $\mathbb{V}_1\otimes\mathbb{V}_2$, one can also define a \emph{product state} for generic mixed states:
\begin{align}
    \rho_\text{product}=\rho_1\otimes\rho_2
\end{align}
and a \emph{separate state}:
\begin{align}
    \rho_\text{separable}=\sum_i p_i\rho_{1,i}\otimes\rho_{2,i},~~~\sum_ip_i=1.
\end{align}
A product state features vanishing mutual information (see next section) between subsystems 1 and 2, while a separable state features vanishing quantum entanglement (e.g. in terms of entanglement negativity, see HW4) between sysbsystems 1 and 2. 


\subsection{Entanglement entropy}

In the Schmidt decomposition described previously, if the density matrix $\hat\rho$ is obtained by a partial trace over $B$ from state $\rho_{AB}$ in the composite system $A\otimes B$:
\begin{align}
...
(3) $S(\hat\rho_A)=S(\hat\rho_B)$, which comes directly from Schmidt decomposition of pure state $\dket{\psi_{AB}}$. If $S(\hat\rho_A)=S(\hat\rho_B)>0$, we say that $A$ and $B$ subsystems are (bipartite) entangled in a pure state $\dket{\psi_{AB}}$.
