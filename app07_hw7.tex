...
\subsection{Solving the Lippmann-Schwinger equation for a delta function potential}

In this problem, we will use the Lippmann-Schwinger equation to solve the eigenstate wavefunctions for scattering states with a delta function potential:
\begin{align}
...
(1) Show the following relation holds for an arbitrary potential $V(\hat x)$
\begin{align}
...
In our lectures, we have derived the asymptotic form of exact eigenstates for an arbitrary localized potential $\lim_{|x|\rightarrow\infty}V(x)=0$:
\begin{align}
...
\end{align}
Diagonalized it to obtain its eigenvalues $e^{2\imth\delta_+}$ and $e^{2\imth\delta_-}$ as a function of $r$ and $t$. 
...
(1) Consider an eigenstate $\dket{\psi_k^+}$ with energy $0<E<U$. Write down the form of its eigenstate wavefunction $\psi_k^+(x)$, without computing the coefficients. 
...
(4) \textbf{Bonus} Show that in the low energy limit of 
\begin{align}
