...
\begin{thm}
  \textbf{Triangle inequality} for the norms of vectors:
\begin{align}
...
\begin{align}
\dket{v}=\sum_iv_i\dket{i},~\forall~\dket{v}\in\mathbb{V};~~~\expval{i|j}=\delta_{i,j},~~~v_i=\expval{i|v}.
\end{align}
...
\end{align}
The column vector $(v_1,\cdots,v_n)$ is known as the \emph{wavefunction} of state $\dket{v}$ in the basis set $\{\dket{i}\}$. 

A special set of complete orthonormal basis that plays a particularly important role in quantum mechanics is the eigenstates $\dket{x}$ of position operator $\hat x$, where we consider one dimension for simplicity. For any normalized state $\dket\psi$, its continuous-variable wavefunction
\begin{align}
\psi(x)\equiv\expval{x|\psi},~~~\int\text{d}x|\psi(x)|^2=\expval{\psi|\psi}=1.
\end{align}
determines the probability of detecting the quantum-mechanical particle at position $x$:
\begin{align}
    \text{Pr}(x)=|\psi(x)|^2,~~~\int\text{d}x\text{Pr}(x)=1.
\end{align}