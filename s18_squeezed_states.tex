...
\end{equation}
the electric field of a propagating (non-standing) wave 
\begin{equation}
 \hat E(\bm r,t)=
 \sqrt{\frac{\hbar\omega}{\epsilon L}}
 \left(\hat a e^{-\imth(\omega t-\bm k\cdot\bm r)}
 -\hat a^\dagger e^{\imth(\omega t-\bm k\cdot\bm r)}\right).
\end{equation}
can be rewritten as a superposition of quadratures:
\begin{equation}
