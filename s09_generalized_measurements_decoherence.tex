...
In a generalized measurement, the set of positive operators $\{E_m\equiv M_m^\dagger M_m\}$ is known as a POVM (positive operator-value measure). A generalized measurement is sometimes also known as a POVM. 

\subsection{Examples of generalized measurements}

(1) \emph{Quantum Zeno effect}

In a single-qubit system, consider an $N$-step procedure in which each step consists of unitary time evolution
for a duration $T/N$, followed by a projective measurement of $\hat X$:
\begin{align}
