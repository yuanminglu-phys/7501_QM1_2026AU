...
(5) \textbf{Bonus} In the Davisson-Germer experiment, the incident momentum $\vec k_\text{in}$ is perpendicular to the (111) surface i.e. $\vec k_i=\frac{2\pi}{\lambda}(1,1,1)/\sqrt{3}$ where $\lambda$ is the wavelength of electrons with a kinetic energy of 54 eV. For elastic scatterings responsible for the diffraction pattern, we have 
...
(6) \textbf{Bonus} Note that the in-plane momentum transfer $\vec q$ can be written as
\begin{align}
...
Consider the Compton effect discussed in lecture notes (Fig. \ref{fig:compton}), where an $X$-ray photon was scattered by a stationary electron. 

(1) Write down the momenta conservation and energy conservation formula. 

(2) Use energy and momentum conservations to derive the Compton shift formula:
\begin{align}
...
(1) Consider the circular motion of an electron around the nucleus of hydrogen atom. Using Bohr's quantization condition of the angular momentum
\begin{align}
    mvr=n\hbar
\end{align}
derive the formula of quantized energy
\begin{align}
E_n=-\frac{mk^2e^4}{2\hbar^2}\frac1{n^2}
\end{align}

(2) Compute the energy
\begin{align}
    |E_1|=\frac{mk^2e^4}{2\hbar^2}
\end{align}
in unit of electron volt (eV), and then in unit of joule (J). 

\section{Mode counting in blackbody radiation}

Consider light waves in a cubic box of size $L\times L\times L$. 

(1) Using the standing wave condition, derive the condition for the wavevector $\vec k=(k_x,k_y,k_z)$:
\begin{align}
    k_i=n_i\frac\pi L,~~~i=x,y,z,~~~n_{x},n_y,n_z=1,2,3,\cdots
\end{align}

(2) \textbf{Bonus} Show that in the large box limit of $L\rightarrow\infty$, the total number of standing wave modes in the box is given by
\begin{align}
    N_\text{modes}=\sum_{n_x,n_y,n_z}=\frac{L^3}{2\pi^2}\int|k|^2\text{d}|k|
\end{align}
