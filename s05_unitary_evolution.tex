...
\end{align}
and the different oscillation frequency $\omega_a=E_a/\hbar$ in different components of state $\dket{\psi(t)}$ can result in oscillations of physical observables. Two such examples, i.e. (1) spin precession; and (2) neutrino oscillation are discussed in detail in \sakurai~2.1. In particular, a physical observable you will encounter a lot in the future is the \emph{transition probability}:
\begin{align}
P(i\rightarrow f)\equiv|\expval{f|e^{-\imth\hat Ht/\hbar}|i}|^2
\end{align}
which measures the probability that an initial state $\dket{i}$ evolves into a final state $\dket{f}$ after time $t$. This quantity corresponds to the observed beam intensity in various scattering experiments.
