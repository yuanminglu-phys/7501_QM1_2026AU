...
(5) \textbf{Bonus} Consider a composite system of two qubits $A$ and $B$. Can you write down the generic form of a maximally entangled state $\dket{\psi_{AB}}$, satisfying that the reduced density matrix has a maximal entanglement entropy?
...
\end{align}
where $\{\alpha_j|j=1,2\}$ is a set of complete orthonormal basis for $\mathbb{C}^2$, and so are $\{\beta_j|j=1,2\}$ and $\{\gamma_j|j=1,2\}$. Find one such state. 


\subsection{Witnessing entanglement}

Consider the singlet Bell state
\begin{align}
    \dket{\epr}=\frac1{\sqrt2}(\dket{01}-\dket{10})
\end{align}
where we have used shorthand notation $\dket{10}\equiv\dket1_1\otimes\dket0_2$ for two qubits 1 and 2. From the lecture, we know that this state is invariant under spin rotations, i.e. 
\begin{align}
    \dket{\epr}=\frac1{\sqrt2}(\dket{+\vec n,-\vec n}-\dket{-\vec n,+\vec n})
\end{align}
where $\vec n$ is any unit vector, and $\dket{\vec n}$ is the eigenstate of $\vec n\cdot\vec\sigma$:
\begin{align}
    \vec n\cdot\hat{\vec\sigma}\dket{\vec n}=+\dket{\vec n}
\end{align}

(1) Show that 
\begin{align}
    \expval{\vec m|\vec n\cdot\hat{\vec\sigma}|\vec m}=\vec m\cdot\vec n
\end{align}
where $\vec m,\vec n$ are both unit vectors. 

(2) For two unit vectors $\vec m$ and $\vec n$, show that
\begin{align}
    \expval{\epr|(\vec m\cdot\vec\sigma_1)\otimes(\vec n\cdot\vec\sigma_2)|\epr}=-\vec m\cdot\vec n
\end{align}

(3) Show that for simultaneous measurements of two compatible observables $\vec m\cdot\hat{\vec\sigma}_1$ and $\vec n\cdot\hat{\vec\sigma}_2$ in the singlet Bell state $\dket\epr$, the probability for the measurement outcome $\vec m\cdot\vec\sigma_1=+1$ and $\vec n\cdot\vec\sigma_2=+1$ is given by
\begin{align}
    P(\vec m,\vec n)=\frac12|\expval{-\vec m|\vec n}|^2
\end{align}
where $\vec m,\vec n$ are unit vectors. 

For $\vec m=(\cos\theta_1,\sin\theta_1,0)$ and $\vec n=(\cos\theta_2,\sin\theta_2,0)$ in $x$-$y$ plane, further show that
\begin{align}
    P(\vec m,\vec n)=\frac12\sin^2\frac{\theta_1-\theta_2}2
\end{align}


(4) \textbf{Bonus} Tsirelson's inequality: \nielsen~Problem 2.3

(5) \textbf{Bonus} As shown in Fig. \ref{fig:hw4 aspect exp}, to understand Aspect's experiment to demonstrate a violation to CHSH inequality in 1982 (Nobel Prize 2022), solve \bebera~problem 15.7 (see Fig. \ref{fig:hw4 aspect exp}).

\begin{figure}
\includegraphics[width=0.99\columnwidth]{figures/hw4_aspect_exp.pdf}
\caption{Problem 3.(2)}
\label{fig:hw4 aspect exp}
\end{figure}


% \subsection{Entanglement negativity (Bonus)}
...
% \end{align}
% Clearly it reduces to (twice) the entanglement entropy $2S(\rho_A)$ in the special case when $\rho_{AB}$ is a pure state.
...
% (2) \textbf{Bonus} Now consider a composite system of two qubits, one qubit in $A$ and one qubit in $B$. Show that for the Bell state $\dket{\epr}$ in this composite system, the logarithmic negativity is $\log2$. 
