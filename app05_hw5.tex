...
(1) As we have learnt in the lecture notes, the 2-step procedure, of unitary time evolution for $\Delta t=T/n$ followed by a projective measurement on $\hat X$, can be described by a generalized measurement. Compute the measurement operators $\{\hat M_m|m=\pm\}$ as $2\times2$ matrices, and show that they satisfy the completeness relation
\begin{align}
...
\end{align}
In the discussion of quantum Zeno effect, such a generalized measurement (without knowing measurement outcomes) was performed $n$ times. Starting from the initial density matrix $\hat\rho_0$ in (\ref{quantum zeno:initial state}), what is the final density matrix $\mathcal{E}^n[\rho_0]$ after $n$ such operation? 

Hint: any single-qubit density matrix can be expressed in terms of the Bloch ball:
\begin{align}
...
(3) Using the above results, compute the probability at time $T$ (after $n$ such 2-step procedures) to produce the measurement outcome of $\sigma_x=+1$. 
