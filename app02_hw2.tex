...
(4) \textbf{Bonus} Assume Hamiltonian $H$ commutes with two linear operators, $A_1$ and $A_2$, which do not commute with each other:
\begin{align}
[H,A_1]=[H,A_2]=0,~~~[A_1,A_2]\neq0
\end{align}
Show that the eigenstates of $H$ are in general degenerate. 

(5) \textbf{Bonus} Assume two observables $A_1$ and $A_2$ anticommute with each other, i.e.
\begin{align}
...
(7) \textbf{Bonus} For two linear operators satisfying $[\hat A,\hat B]=\alpha\hat 1$ where $\hat 1$ is the identity operator, prove the following identity:
\begin{align}
...
\begin{align}
    [\hat x,\hat p]=\imth\hbar=\imth h/2\pi
\end{align}
Use the results of part (7) to show that for every $\hat x$ eigenstate $\dket{x_0}$ with eigenvalue $x_0$ 
\begin{align}
    \hat x\dket{x_0}=x_0\dket{x_0}
\end{align}
the following state
\begin{align}
    \dket{x_0+a}\equiv e^{-\imth\hat p a/\hbar}\dket{x_0}
\end{align}
is also an eigenstate of $\hat x$ with eigenvalue $x_0+a$. 

Comment: physically this means the linear operator $\hat T(a)\equiv e^{-\imth pa/\hbar}$ translate the position by $a$, from $x_0$ to $x_0+a$. 
...
(1) Find a single-qubit state $\dket{\psi}$ that satisfies the condition (\ref{hw3.1}). 

(2) \textbf{Bonus} If the quantum state is a single-qubit mixed state, it is generally described by a density matrix of the following form:
\begin{align}
...
Comment: you will need to learn about mixed states and measurements in \nielsen~2.4.1.
...
\end{align}
at time $t=0$, we evolve the system by time $T/n$, followed by a measurement of $\hat\sigma_x$, and then repeat the same 2-step procedure (evolution by $\frac{T}{n}$ + $\hat\sigma_x$ measurement) $n$ times. 

(1) Starting from the initial state $\dket{+}$, after unitary time evolution of time $T/n$, obtain the new state
\begin{align}
    \dket{\psi_+}=e^{-\imth\hat HT/n\hbar}\dket+
\end{align}

Hint: use the results of problem 1 on Pauli matrices. 

(2) After a measurement of $\hat\sigma_x$ on the above state $\dket{\psi_+}$, what is the probability of measurement outcome $\text{Pr}(\sigma_x=+1)$? What does the state collapse into after reading out the measurement outcome $\sigma_x=+1$?

(3) Consider the same measurement of $\hat\sigma_x$ on the above state $\dket{\psi_+}$, what is the probability of measurement outcome $\text{Pr}(\sigma_x=-1)$? What does the state collapse into after reading out the measurement outcome $\sigma_x=-1$?

(4) \textbf{Bonus} What is the probability at time $T$ (after $n$ such 2-step procedures) to produce the measurement outcome of $\sigma_x=+1$?